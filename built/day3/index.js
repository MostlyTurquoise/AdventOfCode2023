...
function task1() {
    let input = fs.readFileSync("./day3/input.txt").toString();
    let schematic = input.split("\n");
    let sumOfAllParts = 0;
            ...
            if (isNumeric(schematic[y][x])) {
                ...
                for (let yMod = -1; yMod <= 1; yMod++) {
                    for (let xMod = -thisNo.length; xMod <= 1; xMod++) {
                        if (x + xMod < schematic[0].length &&
                            x + xMod > 0 &&
                            y + yMod < schematic.length &&
                            y + yMod > 0) {
                            if (schematic[y + yMod][x + xMod] != "." &&
                                !isNumeric(schematic[y + yMod][x + xMod])) {
                                enginePart = true;
                            }
                            try {
                                process.stdout.write(schematic[y + yMod][x + xMod]);
                            }
                            catch (err) {
                        ...
                        if (x + xMod < schematic[0].length &&
                            ...
                            if (isNumeric(schematic[y + yMod][x + xMod])) {
                                ...
                                while (isNumeric(schematic[y + yMod][x + xMod + noMod + 1])) {
                                    ...
                                    thisNo += schematic[y + yMod][x + xMod + noMod];
                                    try {
                                        process.stdout.write(schematic[y + yMod][x + xMod + noMod]);
                                    }
                                    catch (err) {
                                        process.stdout.write(" ");
                                    }
